...
public class HashedWheelTimer implements ITimer {
    ...
    final IReusableIterator<HashedWheelTimeout>[] iterators;
    final int mask;
...
//        workerThread = threadFactory.newThread( new Thread( worker, "Hashed wheel timer #" + id.incrementAndGet() ) );
        workerThread = new Thread( worker, "Hashed wheel timer #" + id.incrementAndGet()  );
    ...
    private static Set<HashedWheelTimeout>[] createWheel( int ticksPerWheel ) {
        ...
        for( int i = 0; i < wheel.length; i++ ) {
        	...
        	//wheel[i] = new MapBackedSet<HashedWheelTimeout>( new ConcurrentHashMap<HashedWheelTimeout, Boolean>(16, 0.95f, 4) );
        }
        return wheel;
    }

    @SuppressWarnings("unchecked")
    private static IReusableIterator<HashedWheelTimeout>[] createIterators( Set<HashedWheelTimeout>[] wheel ) {
        IReusableIterator<HashedWheelTimeout>[] iterators = new IReusableIterator[wheel.length];
        for( int i = 0; i < wheel.length; i++ ) {
            iterators[i] = (IReusableIterator<HashedWheelTimeout>) wheel[i].iterator();
        }
        return iterators;
    }
    ...
    public synchronized Set<ITimeout> stop() {
    ...
    void scheduleTimeout(HashedWheelTimeout timeout, long delay) {
        ...
        final long lastRoundDelay = delay % roundDuration;//计算小圈内总共所需要的时间,没有转换为具体的桶的数目（最外层的大圈下面考虑）
        final long lastTickDelay = delay % tickDuration;//计算小圈内是否需要加额外1个桶，如果正好除尽则不需要额外加
        final long relativeIndex = lastRoundDelay / tickDuration + (lastTickDelay != 0? 1 : 0);//准确计算小圈内的桶

        final long remainingRounds = delay / roundDuration - (delay % roundDuration == 0? 1 : 0);//计算需要最外层所需要的大圈数
    ...
    private final class Worker implements Runnable {
        ...
        private void fetchExpiredTimeouts( List<HashedWheelTimeout> expiredTimeouts, long deadline) {
            ...
            try {
                ...
                IReusableIterator<HashedWheelTimeout> i = iterators[newWheelCursor];
        ...
        private void fetchExpiredTimeouts( List<HashedWheelTimeout> expiredTimeouts, IReusableIterator<HashedWheelTimeout> i, long deadline ) {
            ...
            while( i.hasNext() ) {
                ...
                if (timeout.remainingRounds <= 0) {
                    ...
                    if (timeout.deadline <= deadline) {
                        ...
                        System.out.println( "调度出错了： " + timeout );
                        slipped.add( timeout );
                    }
                } else {
